...
def write_to_sheet(worksheet, row_number, data):
    ...
    columns = {
        'BF': data.get('민팅 수량', 0),  # Minting Quantity
        'CE': data.get('퀘스트 소각 수량', 0),  # Quest Burn Quantity
        'CJ': data.get('다오 소각 수량', 0),  # Dao Burn Quantity
        # 'CT': data.get('최고 가격(/NFT)', 0)  # Highest Price(/NFT) - Special handling below
    }
    ...
    highest_price_cell = f'CT{row_number}'
    ...
    if highest_price is not None:
        ...
        previous_row_value = worksheet.acell(f'CT{row_number - 1}').value
